...
int ServoL_max = 32; //ServoL_max mean how close to the center the left servo arm gets.
                     // 37 means perpendicular. This should no go above this 37
                     // Decrease this propotionate to ServoR_max for lighter breaths
int ServoR_max = 10; //ServoR_max mean how close to the center the right servo arm gets.
                    //7 means perpendicular this should not go below 7.
...
int rate = 35;
int pause_after_inhale = 500;
int pause_after_exhale = 500;
void setup() {
  Serial.begin(9600);
...
void updateData() {
    ...
    switch (tVolume) {
      case 3:                     // This was updated after re-calibration
        ServoR_max = 7;
        ServoL_max = 37;
        break;
      case 2:                     // This was updated after re-calibration
        ServoR_max = 15;
        ServoL_max = 30;
        break;
      case 1:                     // This was updated after re-calibration
        ServoR_max = 20;   
        ServoL_max =  23;
        break;  
    }
...
void measure() {
    ...
    unsigned long inspiratory = 0;
    unsigned long breath = 0;
    float avgBreath = 0;
    float avgInspiratory = 0;
    int startingTime = 0;
    int endingTime = 0;
    for (rate = 20; rate < 100; rate+=10) {
        /*for (tVolume = 1; tVolume <= 3; tVolume++) {
          switch (tVolume) {
            case 3:                     // This was updated after re-calibration
              ServoR_max = 7;
              ServoL_max = 37;
           ...
           Serial.println("rate: ");
           Serial.println(rate);
           Serial.println("ServoL_max: ");
           Serial.println(ServoL_max);
           Serial.println("ServoR_max: ");
           Serial.println(ServoR_max);
           startingTime = millis();
            for (int i = 0; i < 5; i++) {
                 ...
                 delay(pause_after_exhale);
                 breath = millis();
                 avgBreath += (breath - initialMillis);
                 avgInspiratory += (inspiratory - initialMillis);
            }
            avgBreath /= 5000.0;
            avgInspiratory /= 5000.0;
            endingTime = millis();
            
            Serial.println("inspiratory time: " );
            Serial.println(avgInspiratory);
            Serial.println("bpm: ");
            Serial.println(60.0 / avgBreath);
        }
    }
}
